...
#undef min
    ...
    class vector
    {
        //static_assert:静态断言，用来发现编译期间是否存在错误，static_assert(常量表达式，提示字符串)，当表达式的值为真时，不做任何处理；
        //为FALSE时，给出错误提示。好处如下：
        //1.可以在编译期间发现更多的错误
        //2.static_assert可以用在全局作用域、命名空间、类作用域、函数作用域，几乎不受限制的使用
        //3.编译器在遇到一个static_assert语句时，通常会立即将其第一个参数作为常量表达式进行演算，但若该表达式依赖于某些模板参数
        //则延迟到模板实例化时再进行，使得检查模板参数成为了可能。如此处就是这样
        //4.static_assert在编译期间断言，不生成目标代码，因此不会造成运行期间性能损失
        static_assert(!std::is_same<bool, T>::value, "vector<bool> is abandoned in mystl");
    public:
        // vector 的嵌套型别定义
        //定义一个名为allocator_type的mysql::allocator<T>类型，如typedef int x;定义一个名为x的int类型
        typedef mystl::allocator<T>                      allocator_type;
        typedef mystl::allocator<T>                      data_allocator;

        //typedef创建类型的别名，typename告诉编译器allocator_type::value_type是一个类型
    ...
    public:
        // 构造、复制、移动、析构函数
        //C++11之后，使用noexcept,告诉编译器该函数中不会发生异常，有利于编译器对程序做更多的优化
        //如果在运行时，noexcept函数向外抛出了异常(如果函数内部捕捉并完成处理，这不算抛出异常)，程序会调用std::terminate()函数，该函数内部会调用
        //std::abort()终止程序
        vector() noexcept
        {
            try_init();
        ...
        //进行函数调用时如果参数的类型和函数形参的类型不一致，会进行类型转换，如果不需要这种类型转换，可以使用explicit
        //explicit关键字只能用于类内部的构造函数声明上
        //explicit关键字用于单个参数的构造函数声明上。这里的单个参数并不是指构造函数只有一个形参，构造函数参数可以有多个，但其他的形参都有默认值
